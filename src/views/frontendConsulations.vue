...
import { startSession } from '@/api/frontendAdmin'
import {
  getSessionList,
  getSessionDelete,
  getSessionDetail,
  getSessionEmotion
} from '../api/frontendAdmin'
import robotIcon from '../assets/images/robot-fill.png'
...
const iconUrl = robotIcon
const iconUrl1 = likeIcon
const usersUrl2 = users

const messageList = ref([]) // ✅ 改名避免和局部变量冲突
const currentSession = ref(null)
const sessionList = ref([])
const isAiTying = ref(false)
const userMessage = ref('')
const chatMessagesRef = ref(null) // ✅ 用于自动滚动到底部
...
const getRiskText = (level) => {
  const map = { 0: '正常', 1: '关注', 2: '预警', 3: '消极' }
  return map[level] ?? '正常'
}

const handleSessionClick = (session) => {
  getSessionDetail(session.id).then((res) => {
    messageList.value = res // ✅ 用 messageList
    scrollToBottom()
  })
...
const sendMessage = () => {
  ...
  const text = userMessage.value.trim() // ✅ 用 text，不遮盖 messageList
  userMessage.value = ''
  ...
  if (!currentSession.value || currentSession.value.status === 'TEMP') {
    ...
    startAIResponse(currentSession.value.id, text)
  }
}
...
const startAIResponse = (sessionId, text) => {
  ...
  fetchEventSource('/api/psychological-chat/stream', {
    ...
    onmessage: (event) => {
      ...
      if (!lastMsg || lastMsg.senderType !== 2) return
    ...
    onclose: () => {
      isAiTying.value = false // ✅ 连接关闭时也重置
