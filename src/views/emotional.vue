    ...
    <El-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="会话ID" width="80">
        <template #default="scope">
          <el-avatar>{{ scope.row.moodScore }}</el-avatar>
        </template>
      </el-table-column>
      ...
      <El-table-column prop="emotionalStatus" label="生活指标" width="100">
        <template #default="scope">
          <div>
            <p>睡眠：{{ scope.row.sleepScore }}/5</p>
      ...
      <el-table-column prop="title" label="操作" width="240">
        <template #default="scope">
          <el-button type="success" text @click="viewSessionDetail(scope.row)"
            >详情</el-button
          >
...
const viewSessionDetail = (row) => {
  console.log('aiEmotionAnalysis原始值:', row.aiEmotionAnalysis)
  currentDetail.value = row
  if (row.aiEmotionAnalysis) {
    aiData.value = JSON.parse(row.aiEmotionAnalysis)
    console.log('解析后的aiData:', aiData.value)
  } else {
    aiData.value = {}
  }
...
const handleDelete = async (row) => {
  ElMessageBox.confirm(`确定要删除记录吗？`, '确认', {
